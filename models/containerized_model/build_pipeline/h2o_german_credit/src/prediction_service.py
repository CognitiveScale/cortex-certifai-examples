...
# <Add columns here>
COLUMNS = ['checkingstatus', 'duration', 'history', 'purpose', 'amount', 'savings',
...
def _get_prediction_class(preds):
    ...
    """
    if preds[0] > preds[1]:
        return self.score_labels[0]
    return self.score_labels[1]
...
class MojoModelWrapper(SimpleModelWrapper):
    ...
    def soft_predict(self, npinstances):
        instances = [tuple(instance) for instance in npinstances]
        input_dt = dt.Frame(instances, names=COLUMNS)
        predictions = MODEL.predict(input_dt).to_numpy()
        return predictions
