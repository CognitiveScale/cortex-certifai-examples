...
function train_models() {
  (cd  ../../german_credit && python train.py)
  cp ../../german_credit/german_credit_dtree.pkl sklearn_german_credit/model/model.pkl
}

function sklearn_setup() {
  echo "***Generating ${name}***"
  name=$1
  rm -rf ${GEN_DIR}
  sh ${THIS_DIR}/../generate.sh -i ${name}:latest -m "python_sklearn"

  for f in model/metadata.yml model/model.pkl
  do
    cp ${THIS_DIR}/${name}/${f} ${GEN_DIR}/${f}
  done
...
train_models
for name in sklearn_german_credit
do
  sklearn_setup $name
  build_and_test $name
done
