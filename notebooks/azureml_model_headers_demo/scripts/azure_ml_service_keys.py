...
def main(args):
    ...
    svm_model = build_model(
        (encoder(X_train.values), y_train),
        svm_model_name,
        "SVM",
    ...
    def dump_model(model_name, model_obj, encoder_obj=encoder):
        ...
        model_obj = {"model": model_obj, "encoder": encoder_obj}
    ...
    logistic_model_azure = Model.register(
        model_path=logistic_model_disk_path,
    ...
    svm_model_azure = Model.register(
        model_path=svm_model_disk_path,
    ...
    def get_docker_envars(az_model, suffix):
        return f"azureml-models/{az_model.name}/{az_model.version}"

    inference_config_logistic = InferenceConfig(
        entry_script="logistic_score.py",
        environment=environment,
        source_directory="scripts",
    )

    # inference_config_logistic.environment.environment_variables = {
    #     "AZUREML_MODEL_DIR": get_docker_envars(logistic_model_azure, "logit")
    # }

    inference_config_svm = InferenceConfig(
        entry_script="svm_score.py",
        environment=environment,
        source_directory="scripts",
    )
    # inference_config_svm.environment.environment_variables = {
    ...
    print(
        f"Registering Webservice for {logistic_model_name} with name {logistic_service_name}"
    )
    service_logistic = Model.deploy(
        ws,
    ...
    print(
        'The models and services created by this script will continue running until deleted. If youre following another notebook and ran this script as part of that tutorial, wait till you"ve finished the notebook before cleaning up the resources generated by this notebook'
    )
    ...
    print(
        "You need to take the following values and input them into your Azure Certifai Pro tutorial notebook"
    )
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description="Generate Azure ML Hosted model URIs and Service Keys"
    )
    parser.add_argument(
        "--az_config",
        default="config.json",
        help="Path to the workspace config.json for your Azure Machine Learning Resource. Defaults to ./config.json",
    )
